...
```{r}
Iso  = read.fst('~/Dropbox (Personal)/ProGeny_isochrone/MistIso.fst', as.data.table=TRUE)
#Iso  = read.fst('~/Dropbox (Personal)/ProGeny_isochrone/ParsecIso.fst')
#Iso  = read.fst('~/Dropbox (Personal)/ProGeny_isochrone/BastiIso.fst')
```

If you do not have one already, you can get them using the *progenyIsoDownload* function. Currently we provide MIST (preferred, for reasons we shall get to), PARSEC and BaSTI isochrones.

Here we load four different base spectra options: PHOENIX, C3K, MILES and BASEL. To get these locally you will need to run *progenyAtmosDownload*. The below might take about a minute to run (depending on the speed of your HDD or SSD).
...
Below we interpolate our spectra onto the isochrone grid of Teff, logG and logZ. The below might take a minute or two to run (depending on your CPU speed).

```{r}
Interp_PHOENIX = progenyInterpGrid_All(Iso = Iso, Spec_combine = Spec_PHOENIX)
Interp_C3K = progenyInterpGrid_All(Iso = Iso, Spec_combine = Spec_C3K)
Interp_MILES = progenyInterpGrid_All(Iso = Iso, Spec_combine = Spec_MILES)
Interp_BASEL = progenyInterpGrid_All(Iso = Iso, Spec_combine = Spec_BASEL)
```

We need to update the isochrones with the origin of the 'best' spectra (since we can only have one per isochrone element). The below should be very fast. Note re need the explicit copies of Iso here because of how **data.table** works- the default is to modify by reference, which means they would all end up using the same isochrone 'best' solutions!

```{r}
Iso_PHOENIX = progenyInterpBest(Iso = copy(Iso),
                        Interp_combine = Interp_PHOENIX,
                        label_AGB = 4:5, #7:8 [PC] / 4:5 [Mist]
                        label_white = 6:8 # 9 [PC] / 6:8 [Mist]
                        )

Iso_C3K = progenyInterpBest(Iso = copy(Iso),
                        Interp_combine = Interp_C3K,
                        label_AGB = 4:5, #7:8 [PC] / 4:5 [Mist]
                        label_white = 6:8 # 9 [PC] / 6:8 [Mist]
                        )

Iso_MILES = progenyInterpBest(Iso = copy(Iso),
                        Interp_combine = Interp_MILES,
                        label_AGB = 4:5, #7:8 [PC] / 4:5 [Mist]
                        label_white = 6:8 # 9 [PC] / 6:8 [Mist]
                        )

Iso_BASEL = progenyInterpBest(Iso = copy(Iso),
...
```{r, fig.width=8, fig.height=6}
for(Z in BC03hr$Z){
  magplot(PG_Ch_PHOENIX$Age, PG_Ch_PHOENIX$Zevo[[min(which(PG_Ch$Z >= Z))]]$SMstar, type='l', ylim=c(0,1),
          log='x', xlab='Age / Yr', ylab = 'SMfrac', col='blue')
  lines(BC03hr$Age, BC03hr$Zevo[[min(which(BC03hr$Z >= Z))]]$SMstar, col='black')
...
for(Z in BC03hr$Z){
  ...
  magplot(mag_NUV_r_PHOENIX, mag_r_Ks_PHOENIX, type='l', col='blue', xlim=c(-2,8), ylim=c(-2.5,2), xlab='NUV - r', ylab='r - Ks')
  points(mag_NUV_r_PHOENIX, mag_r_Ks_PHOENIX, cex=seq(0.5,2,len=5), col='blue')
  lines(mag_NUV_r_C3K, mag_r_Ks_C3K, col='darkgreen')
  points(mag_NUV_r_C3K, mag_r_Ks_C3K, cex=seq(0.5,2,len=5), col='darkgreen')
  ...
  legend('bottomright', legend=c('PG_Ch_PHOENIX', 'PG_Ch_C3K', 'PG_Ch_MILES', 'PG_Ch_BASEL', 'BC03'), col=c('blue', 'darkgreen', 'red', 'orange', 'black'), lty=1)
  legend('topright', legend=paste('Z:',Z))
