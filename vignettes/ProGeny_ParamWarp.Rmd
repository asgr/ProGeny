...
```{r}
track = read.table(system.file("extdata", 'track001p0.eep',
  package="ProGeny"))
```
...
Note here since the star is over ~2 solar masses there is no RGBTip phase.

New we plot HR diagram with resolution of 20 (notice glitches near ZAM):
...
Leaving *n_secondary_between* as the default NULL will silently use the same per phase values as Fortran Iso:

```{r}
## Build EEP track with varying secondaries between primaries
eep = build_eep_track(track, prim)
